...
const Orders = () => {
  ...
  const handleUpdateOrderStatus = (
    orderId: string,
    newStatus: "processing" | "cooking" | "ready"
  ) => {
  ...
  useEffect(() => {
    const fetchUserType = async () => {
      ...
      const { data, error } = await supabase
        .from("User") // Adjust if your user type is stored in another table
        .select("type")
        .eq("id", user.id) // Assuming user.id is the primary key
        .single();
              ...
              <div key={order.id} className="border rounded-lg p-4">
                ...
                <div className="space-y-2">
                  {order.items.map((item) => (
                    ...
                    >
                      <span>
                        {mockMenuItems.find((i) => i.id === item.itemId)?.name}{" "}
                        × {item.quantity}
                      </span>
              ...
              <div key={order.id} className="border rounded-lg p-4">
                ...
                <div className="flex justify-end space-x-2">
                  {order.status === "processing" && (
                    <button
                      onClick={() =>
                        handleUpdateOrderStatus(order.id, "cooking")
                      }
